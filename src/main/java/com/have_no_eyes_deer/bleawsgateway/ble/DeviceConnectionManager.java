...
public class DeviceConnectionManager {
    ...
    private GattDataListener gattDataListener;
     ...
     * GATT数据监听器（供上层接收通知数据）
     */
    public interface GattDataListener {
        void onCharacteristicChanged(String deviceAddress, String deviceName, BluetoothGattCharacteristic characteristic);
    }
    ...
    public void setGattDataListener(GattDataListener listener) {
        this.gattDataListener = listener;
    }
    ...
    public String[] getConnectedDeviceAddresses() {
        String[] addresses = activeConnections.keySet().toArray(new String[0]);
        Log.d(TAG, "getConnectedDeviceAddresses: " + addresses.length + " devices");
        Log.d(TAG, "activeConnections: " + activeConnections.keySet());
        return addresses;
    }
    ...
    private final BluetoothGattCallback gattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (deviceName == null || deviceName.trim().isEmpty()) {
                // 回退到已缓存的连接信息名称
                DeviceConnectionInfo cached = connectionInfoMap.get(deviceAddress);
                if (cached != null && cached.deviceName != null && !cached.deviceName.trim().isEmpty()) {
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            // 转发到上层监听器
            if (gattDataListener != null) {
                String deviceAddress = gatt.getDevice().getAddress();
                String deviceName = gatt.getDevice().getName();
                gattDataListener.onCharacteristicChanged(deviceAddress, deviceName, characteristic);
            }
        }
